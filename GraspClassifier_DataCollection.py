...
def run_GUI_Once():
    ...
    objects = [p.loadURDF("urdf/Gripper/pr2_gripper.urdf",
                          initial_position, initial_orientation)]
    pr2_gripper = objects[0]  # Gripper ID
    # Loading Table's urdf
    table = p.loadURDF('urdf/Table/table.urdf', [0.0, -0.4, 0], [0, 0, 0, 1], 0)
    # Loading object
    objID = p.loadURDF("urdf/Object/cube_small.urdf",
    ...
    print("------------------------------Start------------------------------")
    random_grasp_object(objID, pr2_gripper, pr2_cid)
    p.stepSimulation()
    print("------------------------------End------------------------------")
    time.sleep(2)
    input("Enter to quit")
...
def NoGUI_DataCollection(batch_size=100):
    # --------------------Initialization Start--------------------
    # Shut down GUI
    for i in range(batch_size):
        ...
        # Create constraint of gripper
        pr2_cid = p.createConstraint(
            pr2_gripper, -1, -1, -1, p.JOINT_FIXED, [0, 0, 0], [0, 0, 0], [0, 0, 0])
        # --------------------Initialization End--------------------
...
#GUI DataCollection
def GUI_DataCollection(batch_size=100):  # Recommend!
    # --------------------Initialization Start--------------------
    for i in range(batch_size):
        cid = p.connect(p.SHARED_MEMORY)
        if (cid < 0):
